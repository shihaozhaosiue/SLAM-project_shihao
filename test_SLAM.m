...
% === SETUP ===
imageFolder = fullfile(pwd, 'my_office_images');  % Folder for extracted frames
videoFile = fullfile(pwd, 'my_office_video.mp4'); % Video file to record

recordDuration = 5;  % seconds
frameRate = 60;      % target frame rate

% % === STEP 1: Record video from webcam ===
% fprintf('Starting webcam recording...\n');
...
%     frame = snapshot(cam);    % capture color frame
%     writeVideo(writer, frame);
% end
% clear cam;
% close(writer);
% fprintf('Recording complete: %s\n', videoFile);
%%
% === STEP 2: Extract color frames from video ===
if ~exist(imageFolder, 'dir') || isempty(dir(fullfile(imageFolder, '*.png')))
    fprintf('Extracting color frames...\n');
    mkdir(imageFolder);
    ...
    while hasFrame(vid)
        frame = readFrame(vid);
        frameFilename = fullfile(imageFolder, sprintf('frame_%04d.png', frameIdx));
        imwrite(frame, frameFilename);  % Save as color PNG
        frameIdx = frameIdx + 1;
    end
    fprintf('Saved %d frames to: %s\n', frameIdx - 1, imageFolder);
else
    fprintf('Frames already exist. Skipping extraction.\n');
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Load My Own Office Dataset (instead of TUM Dataset)
% Set the base folder and image folder
baseFolder = 'C:\Shihao_research\Robotic Vision-ME-462-001-202515\SLAM project_shihao';
imageFolder = fullfile(baseFolder, 'my_office_images');

% Create an imageDatastore from the saved grayscale frames
imds = imageDatastore(imageFolder, 'FileExtensions', '.png');

% Read the first image to start SLAM
currFrameIdx = 1;
currI = readimage(imds, currFrameIdx);

% Show the first frame (optional, for checking)
figure;
himage = imshow(currI);
title('First Frame from My Office Dataset');
%% Map Initialization
...
focalLength    = [591.0764, 591.8000];           % fx, fy in pixels
principalPoint = [316.7138, 235.0316];           % cx, cy in pixels
imageSize      = [480, 640];                     % height, width from video resolution
...
if isMapInitialized
    ...
    title('Matched Features for Map Initialization');
else
...
% Show matched features used in current frame
firstImage = readimage(imds, 1);
currImage = readimage(imds, currFrameIdx);
figure;
showMatchedFeatures(firstImage, currImage, ...
    prePoints(indexPairs(:,1)), currPoints(indexPairs(:,2)), 'Montage');
title('Matched Features After Refinement');
